...
async function loadPromoProducts() {
    try {
        ...
        if (!response.ok) throw new Error('Gagal memuat produk promo');
        
        const data = await response.json();
        const products = data.data || data;
        
        // Filter produk dengan harga > 100000 untuk promo (simulasi diskon)
        const promoProducts = products.filter(p => parseFloat(p.price) > 100000).slice(0, 6);
        ...
        if (grid) {
            ...
            grid.innerHTML = promoProducts.map(product => {
                const originalPrice = parseFloat(product.price);
                const discountPercent = 50; // Diskon 50%
                const discountedPrice = originalPrice * (1 - discountPercent / 100);
                
                return `
                    <div class="bg-white border-2 border-purple-200 rounded-xl overflow-hidden hover:shadow-lg transition relative">
                        <div class="absolute top-2 right-2 bg-red-500 text-white px-3 py-1 rounded-full text-sm font-bold z-10">
                            -${discountPercent}%
                        </div>
                        <img src="${product.image || 'https://via.placeholder.com/300'}" alt="${product.name}" class="w-full h-40 object-cover cursor-pointer" onclick="viewProduct(${product.id})">
                        <div class="p-4">
                            <h4 class="font-semibold text-gray-900 mb-2">${product.name}</h4>
                            ...
                            <div class="mb-3">
                                <div class="flex items-center gap-2">
                                    <span class="text-xl font-bold text-red-600">${formatCurrency(discountedPrice)}</span>
                                    <span class="text-sm text-gray-400 line-through">${formatCurrency(originalPrice)}</span>
                                </div>
                                <div class="text-xs text-green-600 font-medium mt-1">
                                    <i class="fas fa-check-circle mr-1"></i>
                                    Hemat ${formatCurrency(originalPrice - discountedPrice)}
                                </div>
                            </div>
                            <button onclick="addToCart(event, ${product.id})" class="w-full gradient-bg text-white py-2 rounded-lg font-medium hover:shadow-lg transition">
        ...
        const grid = document.getElementById('promo-products-grid');
        if (grid) {
            grid.innerHTML = '<div class="col-span-2 text-center py-12"><p class="text-red-500">Gagal memuat produk promo</p></div>';
        }
    }
}
